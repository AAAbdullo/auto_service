...
class _AddServiceScreenState extends State<AddServiceScreen> {
  ...
  Future<void> _loadFullServiceDetails() async {
    ...
    try {
      // Use repository to get fresh details
      // We assume serviceToEdit has the correct ID
      final fullDetails = await AutoServicesRepository().getServiceDetails(
        int.parse(widget.serviceToEdit!.id),
      );

      if (fullDetails != null && mounted) {
        setState(() {
          // Clear existing services to avoid duplicates if any were passed
          _services.clear();
          _populateForm(fullDetails);
        });
      } else {
        // Fallback to passed object if fetch fails
        _populateForm(widget.serviceToEdit!);
      }
    } catch (e) {
  ...
  // Changed to accept an argument
  void _populateForm(AutoServiceModel service) {
    ...
    // Gather from both possible fields to be safe
    if (service.extraServices != null) {
      for (var e in service.extraServices!) {
        if (!_services.contains(e.name)) {
          _services.add(e.name);
        }
      }
    }
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (success && _selectedImage != null && serviceId != null) {
        // ... image upload logic ...
        await AutoServicesRepository().addServiceImage(
          serviceId: serviceId,
                  ...
                  Row(
                    children: [
                      ...
                      Container(
                        ...
                        child: IconButton(
                          onPressed: () {
                            if (_serviceInputController.text
                                .trim()
                                .isNotEmpty) {
                              setState(() {
                                _services.add(
              ...
              _buildSection(
                ...
                children: [
                        ...
                        onTap: () async {
                          ...
                          if (result != null) {
                            setState(() {
                              ...
                              _updateMapMarker(); // Update marker on map
                            });
                  ...
                  if (_selectedLat != null && _selectedLon != null) ...[
                    ...
                    // 🗺️ Mini Map Preview - показывает ВЫБРАННОЕ место
                    const SizedBox(height: 16),
                    Container(
                      height: 200,
                      decoration: BoxDecoration(
                      ...
                      child: Stack(
                        children: [
                          YandexMap(
                            // Создаем маркер ПРЯМО ЗДЕСЬ для выбранного места
                            mapObjects: [
                              PlacemarkMapObject(
